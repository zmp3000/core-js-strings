...
function invertCase(str) {
  ...
  const invertedString = invertedCharacters.join('');
...
function getCardId(value) {
  ...
  // Ищем индексы масти и номинала в массивах suits и ranks
  const suitIndex = suits.indexOf(suit);
  const rankIndex = ranks.indexOf(rank);

  // Рассчитываем индекс карты в стандартной колоде
  const cardId = suitIndex * ranks.length + rankIndex;
